...
//seach bar
        if (isset($_POST['subb'])){
          ...
          {
             $sql = "SELECT * FROM profile WHERE job='Android'";
                                 $result = mysqli_query($conn, $sql);}

              elseif($_POST['search']=='it' || $_POST['search']=='front end' )
          {
             $sql = "SELECT * FROM profile WHERE job='IT/Software'";
                                 $result = mysqli_query($conn, $sql);} 
...
$query = "SELECT city,job_section FROM dashboard WHERE email='$user_email'";
...
if ($result2->num_rows > 0) {
    ...
    while ($row = $result2->fetch_assoc()) {
        ...
        $_SESSION['user_job'] = $row["job_section"];
...
$sql = "SELECT details ,city,email FROM profile WHERE city='$user_city' AND job='$user_job' ";
$result = mysqli_query($conn, $sql);
if ($result->num_rows > 0) {
 ...
 <head>
     ...
     <style>
      ...
      <div class="r1">
        <!--<h3><?php/* echo $row['c_name'] ;*/ echo "ahmed"?></h3>-->
              <h6>  Suggestion </h6>
          <p class="r2"><?php echo $row['city'];?> </p>
          <p class="r3"><?php echo $row['details'];?></p>
        for contact: <a href="#"><?php echo $row['email'];?></a>
          <hr>
       </div> 
